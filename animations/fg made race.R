...
complete_box <- readRDS('pfr/data/kickbox.rds')
...
fg_df <- complete_box %>% 
  ...
  summarise(score_fgm = sum(score_fgm, na.rm = T)) %>% 
  ungroup

all_player_weeks <- fg_df %>% 
  group_by(player_id) %>% 
  summarise(
  ...
  group_by(player_id) %>% 
  fill(team, .direction = 'downup') %>% 
  mutate(car_fgm = cumsum(ifelse(is.na(score_fgm), 0, score_fgm))) %>% 
  ungroup
...
viz_df <- all_player_weeks %>% 
  ...
  filter(week_order > 0 & car_fgm > 0)

#viz_df %>% filter(week_order == 0) %>% view

addtl_end_time <- tibble(week_order = max(all_weeks$week_order) + 1:60)
addtl_start_time <- tibble(week_order = -(1:15 - 1))
...
my_ani <- viz_df %>% 
  ggplot(aes(x = car_fgm, y = rank)) +
  geom_point(aes(x = ifelse(week_order <= 20, 28, leader * .9), y = ifelse(rank == 1, 15.5, NA)), color = 'transparent') +
  geom_barh(aes(group = player_id, color = color_key, fill = color_key), stat = 'identity', show.legend = F, size = 2, width = 0.8, alpha = 0.8) +
  geom_richtext(aes(group = player_id, label = paste0(ifelse(status == 'Hall', '**',''),player, ' (',car_fgm,')',ifelse(status == 'Hall', '**','')), x = car_fgm - leader * 0.005, color = status), hjust = 1, size = 11, family = 'MS Reference Sans Serif', show.legend = F, label.colour = 'transparent', fill = 'transparent', label.size = 0, nudge_y = -0.05) +
  geom_image(aes(group = player_id, image = logo_url, x = car_fgm + leader * 0.02), asp = 2.03, size = 0.03) +
  geom_richtext(aes(x = ifelse(week_order <= 20, 28 * 0.9, leader * .9), y = ifelse(rank == 1, 15.5, NA)), label = '<p>HOF Inductees in <b><span style="color:#6d5e00">GOLD</span></b></p>', text.colour = '#00008B', family = font_SB, size = 12, fill = 'grey70', label.padding = unit(c(1.2,1.1,1.1,1.4), "lines"), label.colour = '#00008B', label.size = 1.4, label.r = unit(0.5, "lines"), hjust = 1) +
  scale_y_reverse(expand = expansion(mult = c(0.01, 0.01))) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.04)), breaks = seq(0,800,25), labels = comma_format(accuracy = 1)) +
  ...
  labs(title = 'Most Field Goals Made in the Super Bowl Era',
       subtitle = '{all_weeks$week_name[as.numeric(ifelse(as.numeric(previous_state) < 1, 1, ifelse(as.numeric(previous_state) > max(all_weeks$week_order), max(all_weeks$week_order), previous_state)))]}  |  Starting in Week 1, 1966',
       x = NULL,
       y = NULL) +
  theme(
...
anim_save(filename = 'graphs/car fgm.mp4', animation = animate_SB(my_ani, nframes = 5600, fps = 40, height = 1080, width = 1920, data_home = 'Data: Pro Football Reference', renderer = ffmpeg_renderer(ffmpeg='C:/Program Files/ImageMagick-7.0.7-Q16/ffmpeg.exe')))
system('"C:/Program Files/ImageMagick-7.0.7-Q16/ffmpeg.exe" -i "graphs/car fgm.mp4" -map 0 -c:v libx264 -c:a copy -y "graphs/car fgm working.mp4"')


system('"C:/Program Files/ImageMagick-7.0.7-Q16/ffmpeg.exe" -i "graphs/car fgm w music.mp4" -map 0 -c:v libx264 -c:a copy -y "graphs/car fgm w music working.mp4"')
