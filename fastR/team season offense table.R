...
pbp_df <- readRDS(url(paste0('https://raw.githubusercontent.com/guga31bb/nflfastR-data/master/data/play_by_play_2020.rds'))) %>% add_xpass
...
team_record <- games_df %>% 
  ...
  filter(team==my_team & season == 2020 & !is.na(result)) %>% 
  mutate(
...
games_df %>% 
  filter(season == 2020 & (away_team == my_team | home_team == my_team)) %>% 
  mutate(
  ...
  rbind(., season_tot_df) %>% 
  gt() %>%
  tab_header(title = paste0('2020 ',teams_colors_logos$team_name[which(teams_colors_logos$team_abbr == my_team)] , ' Offense by Game')) %>% 
  cols_label(
    week = 'Wk',
    short_date = 'Date',
    opp = '',
  ...
  tab_style(style = cell_borders(sides  = 'bottom', color = 'darkblue', weight = '2px'), locations = cells_body(rows = 19)) %>% 
  tab_style(style =
    list(
  ...
  text_transform(
    locations = cells_body(vars(opp), rows = 1:19),
  ...
  table_theme_SB %>% 
  brand_table('team off 2020.png', data_home = 'Data: @nflfastR', base_size = 5)
